# Staff Sample Data Seeding — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create a Python XML-RPC script that seeds realistic employee sample data (attendance, leave, expense, payslip) for the `staff/staff` portal user in the Mujimed Odoo 18 instance.

**Architecture:** Single self-contained Python script using Odoo's XML-RPC API (xmlrpc.client). Follows the same pattern as the existing `migrate_helper.py`. The script is idempotent — safe to run multiple times without duplicating data.

**Tech Stack:** Python 3.12 stdlib (xmlrpc.client, datetime, random), Odoo 18 XML-RPC API

**Spec:** `docs/superpowers/specs/2026-05-06-staff-sample-data-design.md`
...
| File | Responsibility |
|------|---------------|
| `mujimed-manifests/seed_sample_data.py` | Main seeding script — connects to Odoo, creates all sample data |

This is a single-file deliverable. No tests (it's a one-shot data script that must be validated against a running Odoo instance).

---

### Task 1: Script Skeleton + Connection

**Files:**
- Create: `mujimed-manifests/seed_sample_data.py`

- [ ] **Step 1: Create script with connection setup and idempotency helper**
...
if __name__ == "__main__":
    url = sys.argv[1] if len(sys.argv) > 1 else ODOO_URL
    uid, models, db, pw = connect(url)
    print("[mujimed] Sample data seeding started...")
```

- [ ] **Step 2: Verify script runs without error (dry connection test)**

Run: `python3 mujimed-manifests/seed_sample_data.py --help 2>&1 || true`
Expected: No syntax errors (connection will fail without live server, that's OK)

- [ ] **Step 3: Commit skeleton**

```bash
git add mujimed-manifests/seed_sample_data.py
git commit -m "feat: add sample data seeder script skeleton"
```

---

### Task 2: Staff User + Employee Creation

**Files:**
- Modify: `mujimed-manifests/seed_sample_data.py`

- [ ] **Step 1: Add user + employee creation function**
...
```bash
git add mujimed-manifests/seed_sample_data.py
git commit -m "feat: add staff user and employee creation"
```

---

### Task 3: Attendance Records (April 2026)

**Files:**
- Modify: `mujimed-manifests/seed_sample_data.py`

- [ ] **Step 1: Add attendance seeding function**
...
def seed_attendance(models, db, uid, pw, emp_id):
    ...
    for day in range(1, 31):
        ...
        co_dt = datetime(2026, 4, day, co_hour, co_min, random.randint(0, 59))
...
```bash
git add mujimed-manifests/seed_sample_data.py
git commit -m "feat: add attendance sample data for April 2026"
```

---

### Task 4: Leave Types, Allocations, and Leave Requests

**Files:**
- Modify: `mujimed-manifests/seed_sample_data.py`

- [ ] **Step 1: Add leave seeding function**
...
def seed_leaves(models, db, uid, pw, emp_id):
    ...
    if not alloc_ids:
        ...
        print(f"[ok] Created and approved 14-day annual leave allocation")
    ...
    print(f"[ok] Created leave 4/14 特休 (approved)")
    ...
    print(f"[ok] Created leave 4/22 病假 (approved)")

    # Leave 3: 特休 4/28-4/29 (pending - stay in confirm state)
    leave3 = create(models, db, uid, pw, 'hr.leave', {
        'employee_id': emp_id,
        'holiday_status_id': annual_lt,
    ...
    print(f"[ok] Created leave 4/28-29 特休 (pending)")
```
...
```bash
git add mujimed-manifests/seed_sample_data.py
git commit -m "feat: add leave types, allocations and requests"
```

---

### Task 5: Expense Products and Expense Records

**Files:**
- Modify: `mujimed-manifests/seed_sample_data.py`

- [ ] **Step 1: Add expense seeding function**
...
```bash
git add mujimed-manifests/seed_sample_data.py
git commit -m "feat: add expense products and records"
```

---

### Task 6: Payroll Structure, Contract, and Payslip

**Files:**
- Modify: `mujimed-manifests/seed_sample_data.py`

- [ ] **Step 1: Add payslip seeding function**
...
def seed_payslip(models, db, uid, pw, emp_id):
    ...
    # ─── Salary Rules ───
    def get_or_create_rule(name, code, category_id, sequence, amount_fix, struct_id):
    ...
    get_or_create_rule('底薪', 'BASIC_SALARY', cat_basic, 1, 32000, struct_id)
    get_or_create_rule('勞保自付', 'LABOR_INS', cat_ded, 10, -1042, struct_id)
    get_or_create_rule('健保自付', 'HEALTH_INS', cat_ded, 11, -372, struct_id)
    get_or_create_rule('勞退自提', 'PENSION', cat_ded, 12, 0, struct_id)
    get_or_create_rule('淨額', 'NET_SALARY', cat_net, 99, 30586, struct_id)
...
```bash
git add mujimed-manifests/seed_sample_data.py
git commit -m "feat: add payroll structure, contract and payslip"
```

---

### Task 7: Finalize Main and Add CLI Args

**Files:**
- Modify: `mujimed-manifests/seed_sample_data.py`

- [ ] **Step 1: Write complete main block with summary**
...
```bash
chmod +x mujimed-manifests/seed_sample_data.py
```

- [ ] **Step 3: Verify script syntax**

Run: `python3 -c "import ast; ast.parse(open('mujimed-manifests/seed_sample_data.py').read()); print('Syntax OK')"`
Expected: `Syntax OK`
...
```bash
git add mujimed-manifests/seed_sample_data.py
git commit -m "feat: finalize seed script with CLI args and summary output"
```
...
After all tasks are complete, the script can be run against a live Odoo instance:

```bash
# Against local/port-forwarded instance
python3 mujimed-manifests/seed_sample_data.py --url http://localhost:8069

# Against production
python3 mujimed-manifests/seed_sample_data.py
```

The script is idempotent — running it twice will skip already-created data.
