...
set -euo pipefail
# =============================================================
# Apporo Hermes 樣板部署腳本
# 基於 Apporo Hermes 的完整配置（白色 Apporo 三角形幾何 logo）
#
# 用法: ./deploy-apporo-hermes.sh <namespace> <domain> [kubectl-context]
# 範例: ./deploy-apporo-hermes.sh clientb-hermes clientb-hermes.woowtech.io woow-k3s
#
# NOTE: For Apporo instances, use Minimax Token Plan key (sk-cp- prefix),
#       NOT pay-as-you-go key.
# =============================================================
...
echo "============================================================"
echo "  Deploying Apporo Hermes Template"
...
for c in d['spec']['template']['spec']['containers']:
    if c['name'] == 'hermes-agent':
        ...
        # Add postStart: remove unused tools + hermes CLI symlink + remove unwanted skills
...
cat "$SCRIPT_DIR/apply_branding_apporo.py" | $KC exec -i deploy/hermes -c hermes-webui -- sh -c 'cat > /home/hermeswebui/.hermes/apply_branding.py'
...
PF_PID=$!
sleep 3
curl -s -c /tmp/apporo-cookie -X POST -H "Content-Type: application/json" \
  -d '{"password":"admin"}' http://localhost:18787/api/auth/login >/dev/null 2>&1
SKILLS=$(curl -s -b /tmp/apporo-cookie http://localhost:18787/api/skills | python3 -c "
import sys,json
try:
...
" 2>/dev/null)
for S in $SKILLS; do
  curl -s -b /tmp/apporo-cookie -X POST -H "Content-Type: application/json" \
    -d "{\"name\":\"$S\",\"enabled\":true}" http://localhost:18787/api/skills/toggle >/dev/null 2>&1
done
kill $PF_PID 2>/dev/null || true
rm -f /tmp/apporo-cookie
...
echo ""
echo "============================================================"
echo "  Apporo Hermes deployed!"
echo "  WebUI URL : https://$DOMAIN"
echo "  Dashboard : https://$DASHBOARD_DOMAIN  (port 9119)"
echo "  Password  : admin"
echo "============================================================"
echo ""
echo ">>> NOTE: Apporo instances must use Minimax Token Plan key"
echo "    (sk-cp- prefix), NOT pay-as-you-go key."
echo ""
