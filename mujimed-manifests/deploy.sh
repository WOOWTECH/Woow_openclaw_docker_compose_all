...
if kubectl rollout status deployment/mujimed-odoo -n "$NAMESPACE" --timeout=900s; then
    ...
    echo "    - Live Chat"
    echo "    - WOOWTECH: Document Management"
    echo "    - WOOWTECH: Color Customizer"
    echo "    - WOOWTECH: Hide Menu User"
    echo "    - WOOWTECH: HR Payslip Monthly Report"
    echo "    - WOOWTECH: LiveChat LINE Integration"
    echo "    - WOOWTECH: Office Enhance"
    echo "    - WOOWTECH: Loyalty Consign (寄賣卡)"
    echo "    - WOOWTECH: Member Center (會員中心)"
    echo ""
    echo "  Taiwan config:"
    echo "    - Language:  Traditional Chinese (zh_TW)"
    echo "    - Currency:  TWD"
    echo "    - Timezone:  Asia/Taipei"
    echo "    - Accounting: Taiwan Chart of Accounts"
    echo ""
else
    echo ""
